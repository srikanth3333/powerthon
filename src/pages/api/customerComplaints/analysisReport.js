      ...
      let data = await collection.aggregate(
        [
            ...
            {$group: {_id:"$sub_category", 
                complaintGreaterThanDays:{$sum:{
                    $cond: { if: { $gt: [ "$delay", 7200 ] }, then: 1, else: 0 },
                }}, 
                complaintLessThanDays:{$sum:{
                    $cond: { if: { $lt: [ "$delay", 7200 ] }, then: 1, else: 0 },
                }},
                count:{$sum:1}}},
                {$project: {_id:1,complaintGreaterThanDays:1,complaintLessThanDays:1,count:1,percentage:{$round:{$multiply:[{$divide:["$count",totalCount]},100]}}}}
        ]).toArray();
